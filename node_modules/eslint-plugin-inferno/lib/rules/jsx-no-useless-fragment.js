...
module.exports = {
  ...
  create(context) {
    ...
    /**
     * Test whether a node is an padding spaces trimmed by inferno runtime.
     * @param {ASTNode} node
     * @returns {boolean}
     */
    ...
    function isChildOfComponentElement(node) {
      return node.parent.type === 'JSXElement'
        ...
        && !jsxUtil.isFragment(node.parent, infernoPragma, fragmentPragma);
    }
    ...
    function canFix(node) {
      ...
      // Not safe to fix `<Eeee><>foo</></Eeee>` because `Eeee` might require its children be a InfernoElement.
    ...
    function getFix(node) {
      ...
      return function fix(fixer) {
        ...
        return fixer.replaceText(node, trimLikeInferno(childrenText));
      };
    }
    ...
    return {
      JSXElement(node) {
        if (jsxUtil.isFragment(node, infernoPragma, fragmentPragma)) {
          checkNode(node);
        }
      },
