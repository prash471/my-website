...
function createDevToolsBridge() {
    ...
    var oldRenderComplete = options.renderComplete;
        ...
        if (!isInvalid(rootInput)) {
            ...
            if (parentDOM.$V) {
                ...
                for (root in roots) {
                    ...
                    var rootNode = findDOMfromVNode(rootInstance.vNode, true);
                ...
                var dom = findDOMNode(rootInput);
                if (isDetached(dom)) {
                    for (root in roots) {
                        var rootInstance$1 = roots[root];
                        if (isDetached(findDOMNode(rootInstance$1.vNode))) {
